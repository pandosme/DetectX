...
./build.sh
```
This creates `.eap` files (Axis application packages) in the root directory, ready for installation on cameras.

**Prepare model configuration:**
```bash
python prepare.py
```
Run this after replacing the TFLite model or labels to update `app/model/model.json` with correct model parameters.
...
- `Model_Setup()`: Initializes model, reads `model.json`, allocates larod tensors
...
**app/model/model.json** (auto-generated by prepare.py)
- Model parameters: width, height, quantization scale/zero-point, number of boxes/classes
- Video capture dimensions
- Target chip (axis-a8-dlpu-tflite or a9-dlpu-tflite)
- Label array
...
Follow the YOLOv5 training process documented in [Train-Build.md](Train-Build.md):
...
5. Run `python prepare.py` to regenerate `app/model/model.json`
6. Run `./build.sh`
...
- Set in model.json via prepare.py
