...
`onnx2tf -onimc`, quantizes to full INT8, and verifies the result. The same command
works for ARTPEC-8 and ARTPEC-9 — the output format is identical.
...
```bash
cp best_export.tflite app/model/model.tflite
...
parameters — input size, tensor shapes, quantization scales — are extracted
automatically at build time, so nothing needs editing by hand.
